...
std::vector<std::string> gui::logMessages;
...
// Handle ImGui setup and cleanup
...
// Log system
void gui::AddLog(const std::string& message) {
...
void gui::Render() noexcept {
    ...
    ImGui::Begin("External Cheat", &exit, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove);

    if (ImGui::Button("Attach to Game")) cheat::AttachToGame();
    if (ImGui::Checkbox("God Mode", &cheat::GodMode)) cheat::ToggleGodMode();
    if (ImGui::Checkbox("Infinite Grenades", &cheat::InfGrenade)) cheat::ToggleInfGrenade();
    if (ImGui::Checkbox("Infinite AR Ammo", &cheat::InfArAmmo)) cheat::ToggleInfArAmmo();
    if (ImGui::Checkbox("Infinite Jump", &cheat::InfJump)) cheat::ToggleInfJump();
    if (ImGui::Checkbox("No Recoil", &cheat::NoRecoil)) cheat::ToggleNoRecoil();

    ImGui::Text("Log:");
    ImGui::BeginChild("LogBox", ImVec2(0, 100), true);
    for (const std::string& log : logMessages) ImGui::TextUnformatted(log.c_str());
    ImGui::EndChild();
